#!/usr/bin/env python3
"""Fine-tune MiniLM for CVE description + KeyBERT phrases -> CWE.

Standalone trainable-encoder counterpart of this directory's
``NN_keywords.py``. Each phrase is encoded separately, mean-pooled per CVE,
L2-normalised, and concatenated with the normalised description embedding.
Only MiniLM's frozen status changes from the baseline method.
"""
...
OUTPUT_DIR = OUTPUT_ROOT / "CVE_to_CWE" / "NN_fine_tune_keywords"
...
def parse_keyphrases(value: Any) -> list[str]:
    """Parse list-like or delimited keyphrases and remove duplicates."""
    if pd.isna(value):
        return []
    text = str(value).strip()
    ...
    try:
        ...
        parsed = None
    values = (
        list(parsed) if isinstance(parsed, (list, tuple, set))
        else re.split(r"[;,|\n]+", text)
    )
    phrases, seen = [], set()
    for value in values:
...
def load_split(path: Path, split_name: str) -> pd.DataFrame:
    ...
    required = {CVE_COLUMN, DESCRIPTION_COLUMN, KEYPHRASE_COLUMN, LABEL_COLUMN}
    ...
    frame["keyphrase_list"] = frame[KEYPHRASE_COLUMN].map(parse_keyphrases)
    frame["has_keyphrases"] = frame["keyphrase_list"].map(bool)
    return frame
...
class DescriptionKeywordDataset(Dataset):
    def __init__(self, frame: pd.DataFrame, targets: np.ndarray):
        self.descriptions = frame[DESCRIPTION_COLUMN].tolist()
        self.keyphrases = frame["keyphrase_list"].tolist()
        self.has_keyphrases = frame["has_keyphrases"].astype(bool).tolist()
    ...
    def __getitem__(self, index: int) -> tuple[str, list[str], bool, np.ndarray]:
        return (
            self.descriptions[index], self.keyphrases[index],
            self.has_keyphrases[index], self.targets[index],
        )
...
class BatchCollator:
    ...
    def __call__(
        self, rows: Sequence[tuple[str, list[str], bool, np.ndarray]],
    ) -> dict[str, torch.Tensor]:
        descriptions, phrase_lists, has_keyphrases, targets = zip(*rows)
        description_tokens = self.tokenizer(
            list(descriptions), padding=True, truncation=True,
            max_length=self.config.description_max_length, return_tensors="pt",
        )
        phrases, phrase_rows = [], []
        for row_index, row_phrases in enumerate(phrase_lists):
            phrases.extend(row_phrases)
            phrase_rows.extend([row_index] * len(row_phrases))
        # Keep an empty dummy tokenization for a batch containing no phrases.
        if not phrases:
            phrases, phrase_rows = [""], [-1]
        phrase_tokens = self.tokenizer(
            phrases, padding=True, truncation=True,
            max_length=self.config.keyphrase_max_length, return_tensors="pt",
        )
        return {
            "description_input_ids": description_tokens["input_ids"],
            "description_attention_mask": description_tokens["attention_mask"],
            "keyphrase_input_ids": phrase_tokens["input_ids"],
            "keyphrase_attention_mask": phrase_tokens["attention_mask"],
            "keyphrase_row_indices": torch.tensor(phrase_rows, dtype=torch.long),
            "has_keyphrases": torch.tensor(has_keyphrases, dtype=torch.float32),
...
class DescriptionKeywordMiniLMClassifier(nn.Module):
    ...
    def forward(self, batch: dict[str, torch.Tensor]) -> torch.Tensor:
        description_output = self.encoder(
            input_ids=batch["description_input_ids"],
            attention_mask=batch["description_attention_mask"],
        )
        descriptions = torch.nn.functional.normalize(self.mean_pool(
            description_output, batch["description_attention_mask"]
        ), dim=1)
        phrase_output = self.encoder(
            input_ids=batch["keyphrase_input_ids"],
            attention_mask=batch["keyphrase_attention_mask"],
        )
        phrases = torch.nn.functional.normalize(self.mean_pool(
            phrase_output, batch["keyphrase_attention_mask"]
        ), dim=1)

        keywords = torch.zeros(
            (descriptions.shape[0], phrases.shape[1]),
            dtype=phrases.dtype, device=phrases.device,
        )
        counts = torch.zeros(
            descriptions.shape[0], dtype=phrases.dtype, device=phrases.device,
        )
        valid = batch["keyphrase_row_indices"] >= 0
        if bool(valid.any()):
            row_indices = batch["keyphrase_row_indices"][valid]
            keywords = keywords.index_add(0, row_indices, phrases[valid])
            counts.index_add_(
                0, row_indices, torch.ones_like(row_indices, dtype=phrases.dtype)
            )
        keywords = keywords / counts.clamp_min(1.0).unsqueeze(1)
        keywords = torch.nn.functional.normalize(keywords, dim=1)
        keywords = keywords * batch["has_keyphrases"].unsqueeze(1)
        return self.classifier(torch.cat([descriptions, keywords], dim=1))
...
def train_and_evaluate() -> Path:
    ...
    train_loader = DataLoader(
        DescriptionKeywordDataset(train, y_train), batch_size=config.train_batch_size,
        shuffle=True, collate_fn=collator, generator=generator,
    )
    validation_loader = DataLoader(
        DescriptionKeywordDataset(validation, y_validation),
        batch_size=config.eval_batch_size, shuffle=False, collate_fn=collator,
    )
    test_loader = DataLoader(
        DescriptionKeywordDataset(test, y_test), batch_size=config.eval_batch_size,
        shuffle=False, collate_fn=collator,
    )

    model = DescriptionKeywordMiniLMClassifier(config, len(labels)).to(device)
    ...
    for epoch in range(1, config.epochs + 1):
        ...
        if score > best_score:
            ...
            torch.save({
                ...
                "target": "cwe", "use_keywords": True,
                "feature_mode": "cleaned_description+keywords",
                "embed_model_name": config.model_name, "input_dim": model.input_dim,
                "best_epoch": epoch, "threshold": threshold,
    ...
    save_json(OUTPUT_DIR / "metrics.json", {
        "completed": True, "model": "NN_fine_tune_keywords", "target": "cwe",
        "source_splits": str(DATA_DIR), "keyword_method": "keybert_modified",
        "selection_split": "validation", "selection_metric": "micro_f1",
